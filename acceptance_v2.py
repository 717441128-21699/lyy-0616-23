...
def test_issue_1_dual_instance():
    ...
    print(f"  [期望] ≤ {limit + 1} (滑动窗口允许10%误差)")

    if max_in_window > limit + 1:
        print(f"  ❌ 失败：1 秒内通过 {max_in_window} > {limit + 1}")
        return False

    # 检查总速率
    test_duration = 2.5
    rate = total / test_duration
    expected_rate_upper = limit * 1.2
    print(f"  [速率] 实际 {rate:.1f}/s, 期望 ≤ {expected_rate_upper:.1f}/s")

    if rate > expected_rate_upper:
        print(f"  ❌ 失败：总速率 {rate:.1f}/s 超过上限 {expected_rate_upper:.1f}/s")
        return False

    print(f"  ✅ 通过：双实例总通过数在任意 1 秒内都不超过 {limit + 1}，总速率合理")
    print()
    return True
...
def test_issue_2_stats_accuracy():
    ...
    # 检查 5: remaining 计算正确
    expected_remaining = limit - s2["global_count"]
    if s2["remaining"] != expected_remaining:
        errors.append(f"remaining={s2['remaining']} ≠ {expected_remaining}")
    else:
        print(f"  ✅ remaining = {expected_remaining}，计算正确")
